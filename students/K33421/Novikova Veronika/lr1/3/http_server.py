...
class MyHTTPServer:
    ...
    def handle_request(self, req):
        ...
        if req.path.startswith('/marks/'):
            mark_id = req.path[len('/marks/'):]
            if mark_id.isdigit():
                return self.handle_get_mark(req, mark_id)
        raise HTTPError(404, 'Not found')
    ...
    def handle_post_marks(self, req):
        ...
        self._marks[mark_id] = {'id': mark_id,
    ...
    def handle_get_mark(self, req, mark_id):
        mark = self._marks.get(int(mark_id))
        if not mark:
            raise HTTPError(404, 'Not found')
        accept = req.headers.get('Accept')
        if 'text/html' in accept:
            contentType = 'text/html; charset=utf-8'
            body = '<html><head></head><body>'
            body += f'{mark["id"]} {mark["subject"]}: {mark["mark"]}'
            body += '</body></html>'
        else:
            return Response(406, 'Not Acceptable')
        body = body.encode('utf-8')
