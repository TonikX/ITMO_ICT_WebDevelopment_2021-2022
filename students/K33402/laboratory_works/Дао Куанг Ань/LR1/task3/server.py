...
class MyHTTPServer:
    ...
    def handle_post_users(self, req):
        subject_id = len(self._users) + 1
        self._users[subject_id] = {'id': subject_id,
    ...
    def handle_get_users(self, req):
        ...
        if 'text/html' in accept:
            ...
            body += f'<div> Дисциплины ({len(self._users)})</div>'
            body += '<ul>'
    ...
    def handle_get_user(self, req, user_id):
        ...
        if 'text/html' in accept:
            contentType = 'text/html; charset=utf-8'
            body = '<html><head></head><body>'
            body += f'#{user["id"]} {user["subject"]}, {user["grade"]}'
            body += '</body></html>'
...
if __name__ == '__main__':
    serv = MyHTTPServer("127.0.0.1", 8088, "example.local")
    try:
        serv.serve_forever()
