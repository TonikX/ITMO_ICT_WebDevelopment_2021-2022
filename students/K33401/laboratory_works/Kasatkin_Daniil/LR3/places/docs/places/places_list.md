Выводит информацию обо всех местах

**URL** : `/places/`

**Method** : `GET`

**Auth required** : NO

**Permissions required** : None

**Data constraints** : `{}`

## Success Responses

**Code** : `200 OK`

**Content** : `[{}]`

```json
[
    {
        "id": 4,
        "host": {
            "id": 1,
            "password": "pbkdf2_sha256$320000$WqfGI7IC1P4MhfWFYwx4RV$c1JFPxsv3mzJe7RBcSDVBLX4b2jOSkuECL3vmXLZUwc=",
            "last_login": "2022-01-04T10:05:13.434077Z",
            "is_superuser": true,
            "phone_number": "+79630983774",
            "email": "a@a.com",
            "first_name": "admin",
            "join_date": "2022-01-04T10:05:02.629885Z",
            "is_staff": true,
            "is_host": true,
            "is_active": true,
            "groups": [],
            "user_permissions": []
        },
        "place_type": "House",
        "place_offers": "Wifi, Kitchen, Free parking on premises, Shampoo, Hot water, Body soap, Body Washer, Bed linens, Smoke alarm, Cooking basics, TV, Air conditioning, Hair dryer",
        "title": "YOUR winter home in St Petersburg,FL",
        "desc": "The perfect winter getaway. This home in St Petersburg, is close to 275, with a short trip to the beaches, downtown and shopping.\nAll you need to do is pack your toothbrush and a few clothes, then ENJOY.",
        "beds": 3,
        "bedrooms": 2,
        "bathrooms": 2,
        "guests": 4,
        "location": "Florida, United States",
        "image": "http://127.0.0.1:8000/images/places/YOUR%20winter%20home%20in%20St%20Petersburg%2CFL/d772d338-6ac3-4ed7-9cc5-43706fd7181e.webp",
        "price": 5360
    },
    {
        "id": 5,
        "host": {
            "id": 1,
            "password": "pbkdf2_sha256$320000$WqfGI7IC1P4MhfWFYwx4RV$c1JFPxsv3mzJe7RBcSDVBLX4b2jOSkuECL3vmXLZUwc=",
            "last_login": "2022-01-04T10:05:13.434077Z",
            "is_superuser": true,
            "phone_number": "+79630983774",
            "email": "a@a.com",
            "first_name": "admin",
            "join_date": "2022-01-04T10:05:02.629885Z",
            "is_staff": true,
            "is_host": true,
            "is_active": true,
            "groups": [],
            "user_permissions": []
        },
        "place_type": "House",
        "place_offers": "Kitchen, Free parking on premises, Air conditioning, Shampoo, Body Washer, Bed linens",
        "title": "Casa Del Prince Upper",
        "desc": "Come enjoy the unique Shabby chic sleeping and sitting area furnishings in French design motif. Reminiscent of New Orleans; all in new construction Studio apartments. Brilliant Brass decorative touch's; will remind you of Louie Armstrong's trumpet! Apartments feature all the comforts of home, including your own in-unit washer and dryer. Located in the quiet, convenient and historic Euclid St. Paul / Crescent Lake neighborhood of St. Petersburg.",
        "beds": 1,
        "bedrooms": 1,
        "bathrooms": 1,
        "guests": 2,
        "location": "Saint Petersburg",
        "image": "http://127.0.0.1:8000/images/places/Casa%20Del%20Prince%20Upper/2.webp",
        "price": 8782
    },
    {
        "id": 8,
        "host": {
            "id": 1,
            "password": "pbkdf2_sha256$320000$WqfGI7IC1P4MhfWFYwx4RV$c1JFPxsv3mzJe7RBcSDVBLX4b2jOSkuECL3vmXLZUwc=",
            "last_login": "2022-01-04T10:05:13.434077Z",
            "is_superuser": true,
            "phone_number": "+79630983774",
            "email": "a@a.com",
            "first_name": "admin",
            "join_date": "2022-01-04T10:05:02.629885Z",
            "is_staff": true,
            "is_host": true,
            "is_active": true,
            "groups": [],
            "user_permissions": []
        },
        "place_type": "House",
        "place_offers": "Wifi, Free parking on premises",
        "title": "Comfy, Cozy and Centrally Located.",
        "desc": "This one bedroom apartment is centrally located, close to downtown, parks, shopping, the airport, public transportation and beaches. The apartment is pet friendly with it's own enclosed yard for you and your pets to enjoy and offers plenty of privacy. The apartment is perfect for solo travelers, couples, families and business travelers.",
        "beds": 3,
        "bedrooms": 1,
        "bathrooms": 1,
        "guests": 4,
        "location": "Saint Petersburg",
        "image": "http://127.0.0.1:8000/images/places/Comfy%2C%20Cozy%20and%20Centrally%20Located./4.webp",
        "price": 4715
    },
    {
        "id": 9,
        "host": {
            "id": 1,
            "password": "pbkdf2_sha256$320000$WqfGI7IC1P4MhfWFYwx4RV$c1JFPxsv3mzJe7RBcSDVBLX4b2jOSkuECL3vmXLZUwc=",
            "last_login": "2022-01-04T10:05:13.434077Z",
            "is_superuser": true,
            "phone_number": "+79630983774",
            "email": "a@a.com",
            "first_name": "admin",
            "join_date": "2022-01-04T10:05:02.629885Z",
            "is_staff": true,
            "is_host": true,
            "is_active": true,
            "groups": [],
            "user_permissions": []
        },
        "place_type": "Apartment",
        "place_offers": "Kitchen, Pets allowed, Body soap, Hot water",
        "title": "Luxury Living, 2Blocks to Trop, Walking DistanceC",
        "desc": "Visit St Petersburg with Luxury! Walking distance to everything Downtown St Pete has to offer! Walking distance to Tropicana Field! All the culture and food you need, just walking down the street. 1 Mile from the St Pete Pier. 10-15 min drive to the beach. And all these Amenities, including a roof top pool with a skyline view of the city, that this great building has to offer!",
        "beds": 1,
        "bedrooms": 1,
        "bathrooms": 1,
        "guests": 4,
        "location": "St Petersburg",
        "image": "http://127.0.0.1:8000/images/places/Luxury%20Living%2C%202Blocks%20to%20Trop%2C%20Walking%20DistanceC/5.webp",
        "price": 5743
    },
]
```