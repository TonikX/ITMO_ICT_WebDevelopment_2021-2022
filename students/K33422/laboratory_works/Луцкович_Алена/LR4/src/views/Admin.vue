<template>
  <v-data-table
      :headers="headers"
      :items="jobs"
      class="elevation-1"
  >
    <template v-slot:top>
      <v-toolbar
          flat
      >
        <v-spacer></v-spacer>
        <v-dialog
            v-model="dialog"
            max-width="500px"
        >
          <template v-slot:activator="{ on, attrs }">
            <v-btn
                color="orange darken-2"
                dark
                class="mb-2"
                v-bind="attrs"
                v-on="on"
            >
              Add job
            </v-btn>
          </template>
          <v-card>
            <v-card-title>
              <span class="headline">{{ formTitle }}</span>
            </v-card-title>

            <v-card-text>
              <v-container>
                <v-row>
                  <v-col
                      cols="12"
                      sm="6"
                      md="4"
                  >
                    <v-text-field
                        v-model="editedItem.employee"
                        label="Работник"
                    ></v-text-field>
                  </v-col>
                  <v-col
                      cols="12"
                      sm="6"
                      md="4"
                  >
                    <v-text-field
                        v-model="editedItem.job"
                        label="Должность"
                    ></v-text-field>
                  </v-col>
                  <v-col
                      cols="12"
                      sm="6"
                      md="4"
                  >
                    <v-text-field
                        v-model="editedItem.clearance"
                        label="Допуск"
                    ></v-text-field>
                  </v-col>
                  <v-col
                      cols="12"
                      sm="6"
                      md="4"
                  >
                  </v-col>
                </v-row>
              </v-container>
            </v-card-text>

            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn
                  color="red darken-1"
                  text
                  @click="close"
              >
                Cancel
              </v-btn>
              <v-btn
                  color="green darken-4"
                  text
                  @click="save"
              >
                Save
              </v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="dialogDelete" max-width="500px">
          <v-card>
            <v-card-title class="headline">Please confirm your intention.</v-card-title>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="orange darken-1" text @click="closeDelete">Cancel</v-btn>
              <v-btn color="red darken-1" text @click="deleteItemConfirm">Delete</v-btn>
              <v-spacer></v-spacer>
            </v-card-actions>
          </v-card>
        </v-dialog>
      </v-toolbar>
    </template>
    <template v-slot:item.actions="{ item }">
      <v-icon
          small
          class="mr-2"
          @click="editItem(item)"
      >
        mdi-pencil
      </v-icon>
      <v-icon
          small
          @click="deleteItem(item)"
      >
        mdi-delete
      </v-icon>
    </template>
  </v-data-table>
</template>

<script>
import axios from 'axios'
export default {
  data: () => ({
    dialog: false,
    dialogDelete: false,
    headers: [
      {
        text: 'Работник',
        align: 'start',
        sortable: false,
        value: 'employee'
      },
      { text: 'Должность', value: 'job' },
      { text: 'Допуск', value: 'clearance' },
      { text: 'Редактирование', value: 'actions', sortable: false }
    ],
    jobs: [],
    editedIndex: -1,
    editedItem: {
      employee: '',
      job: '',
      clearance: ''
    },
    defaultItem: {
      employee: '',
      job: '',
      clearance: ''
    }
  }),
  computed: {
    formTitle () {
      return this.editedIndex === -1 ? 'New job' : 'Edit job'
    }
  },
  watch: {
    dialog (val) {
      val || this.close()
    },
    dialogDelete (val) {
      val || this.closeDelete()
    }
  },
  created () {
    this.JobsList()
  },
  methods: {
    async JobsList () {
      try {
        const response = await axios
            .get('http://127.0.0.1:8000/airline_admin/')
        if (response.status !== 200) {
          throw new Error(response.status)
        }
        this.jobs = response.data
        return response.data
      } catch (e) {
        console.error('AN API ERROR', e)
      }
    },
    editItem (item) {
      this.editedIndex = this.jobs.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialog = true
    },
    deleteItem (item) {
      this.editedIndex = this.jobs.indexOf(item)
      this.editedItem = Object.assign({}, item)
      this.dialogDelete = true
    },
    deleteItemConfirm () {
      if (this.editedIndex > -1) {
        this.jobs.splice(this.editedIndex, 1)
        this.closeDelete()
        axios.delete('http://127.0.0.1:8000/airline_admin/' + JSON.stringify(this.editedItem.id), {
          job: this.editedItem.job,
          clearance: this.editedItem.clearance
        })
      } else {
        this.jobs.push(this.editedItem)
      }
      this.close()
    },
    close () {
      this.dialog = false
      this.$nextTick(() => {
        this.editedItem = Object.assign({}, this.defaultItem)
        this.editedIndex = -1
      })
    },
    closeDelete () {
      this.dialogDelete = false
      this.$nextTick(() => {
        this.editedItem = Object.assign({}, this.defaultItem)
        this.editedIndex = -1
      })
    },
    async createJob () {
      try {
        const response = await this.axios
            .post('http://localhost:8000/airline_admin/create/', {
              employee: this.editedItem.employee,
              job: this.editedItem.job,
              clearance: this.editedItem.clearance
            })
        if (response.status !== 201) {
          throw new Error(response.status)
        }
        window.location.reload()
      } catch (e) {
        console.error('API ERROR', e)
      }
    },
    async updateJob () {
      this.editedIndex = 1
      try {
        const response = await this.axios
            .put('http://localhost:8000/airline_admin/' + JSON.stringify(this.editedItem.id) + '/', {
              employee: this.editedItem.employee,
              job: this.editedItem.job,
              clearance: this.editedItem.clearance
            })
        if (response.status !== 200) {
          throw new Error(response.status)
        }
        window.location.reload()
      } catch (e) {
        console.error('API ERROR', e)
      }
    },
    save () {
      if (this.editedIndex > -1) {
        this.updateJob()
      } else {
        this.createJob()
      }
    }
  }
}
</script>
<style scoped>
</style>
